...
import { Asistencia, AsistenciaDocument } from './schema/asistencia.schema';
...
export class AsistenciasService {
  ...
  async actualizarAsistencia(usuarioId:any, usuarioNombre:string, nivel:number, id:string, estado:string, observacion?:string): Promise<Asistencia> {
    ...
    if (nivel === niveles.ASIGNADOS && hora > 9) throw new HttpException('La asistencia solo se puede actualizar hasta las 9:00.', HttpStatus.BAD_REQUEST);
  ...
  async listarAsistenciaSupervisor(usuarioId:string, area:string, nivel: number): Promise<Asistencia[]> {
    ...
    console.log(fechaInicio.toJSDate(), fechaFin.toJSDate());
